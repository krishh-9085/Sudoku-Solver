...
async function getPuzzle(difficulty) {
  try {
    showLoadingAnimation(); // Show loading animation while fetching puzzle
    const response = await fetchWithTimeout(
      `https://sugoku.onrender.com/board?difficulty=${difficulty}`,
      {},
      5000 // Set timeout of 5 seconds
    );
    ...
    const puzzle = data.board; // Get the board from the fetched data
    loadPuzzle(puzzle);
  } catch (error) {
    hideLoadingAnimation(); // Hide loading icon on error
    console.error("Error fetching puzzle:", error); // Log the error if fetching fails
    alert("Failed to load puzzle. Please try again."); // Alert user about the error
  }
}
...
function showLoadingAnimation() {
  ...
  const cells = sudokuGrid.children; // Get all grid cells
  for (let i = 0; i < cells.length; i++) {
    const cell = cells[i]; // Get each cell
    const row = Math.floor(i / 9); // Calculate the row index (0 to 8)
    const col = i % 9; // Calculate the column index (0 to 8)

    // Apply animation with delay based on the diagonal wave effect
    const delay = (row + col) * 0.05 + "s"; // Stagger the delay based on the sum of row and column
    cell.style.animation = `popIn 0.5s ease-in-out forwards ${delay}`; // Apply the animation with the delay
  }
}

// Function to hide the loading animation
// Function to hide the loading animation with a delay// Function to hide the loading animation with a delay
function hideLoadingAnimation() {
  const animationDuration = 0.5; // Animation duration in seconds (same as popIn animation duration)
  const delayBeforeHiding = 700; // Delay in milliseconds before hiding the animation
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < cells.length; i++) {
      const cell = cells[i]; // Get each cell
      cell.style.animation = ""; // Reset animation
    }
  }, delayBeforeHiding); // Wait for the delay before hiding
}

// Function to trigger the puzzle fetch after animation finishes
async function startPuzzleFetch(difficulty) {
  const animationDuration = showLoadingAnimation(); // Show loading animation and get its total duration
  
  // Wait for the animation to finish before calling getPuzzle
  setTimeout(() => {
    getPuzzle(difficulty); // Fetch the puzzle after the animation duration
    displayDifficulty(difficulty); // Display the selected difficulty after animation
  }, animationDuration * 1000); // Delay the fetch in milliseconds
}

// Function to display difficulty after animation
function displayDifficulty(difficulty) {
  const difficultyDisplay = document.getElementById("difficultyDisplay"); // Get the element where difficulty will be displayed
  difficultyDisplay.textContent = `Difficulty: ${difficulty}`; // Set the difficulty text
}

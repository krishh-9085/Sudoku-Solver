...
async function getPuzzle(difficulty) {
  const cachedPuzzle = localStorage.getItem(difficulty); // Check for cached puzzle
  if (cachedPuzzle) {
    // Use cached puzzle
    ...
    try {
      loadingIcon.style.display = "block"; // Show loading icon while fetching puzzle
      ...
      localStorage.setItem(difficulty, JSON.stringify(puzzle)); // Cache the puzzle for future use
      loadPuzzle(puzzle);
    } catch (error) {
